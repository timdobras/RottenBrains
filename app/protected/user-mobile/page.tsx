...
const page = async () => {
  ...
  // Fetch history and all watch list categories in parallel
  const [history, watching, planned, watched] = await Promise.all([
    getWatchHistoryForUser(user.id.toString(), 10, 0),
  ...
  return (
    <>
      ...
      <div className="mt-4 flex flex-row items-center justify-between px-4">
        ...
        <Link href="/protected/watch-history" className="text-xs text-foreground/70">
          View all
        </Link>
      </div>
      ...
      <div className="mt-4 flex flex-row items-center justify-between px-4">
        ...
        <Link href="/protected/watch-list" className="text-xs text-foreground/70">
          View all
        </Link>
      </div>

-- Fix: Videasy sends absolute percentage (like Jellyfin), not incremental.
--
-- The Videasy player reports real playback position via postMessage.
-- We calculate absolute percentage from currentTime/duration, so the RPC
-- should use GREATEST (take the higher value) instead of accumulating.
--
-- Treat 'videasy' sync_source the same as 'jellyfin' for percentage and time.
...
BEGIN
    ...
    DO UPDATE SET
        time_spent = CASE
            -- Jellyfin and Videasy send absolute time; use the greater value
            WHEN EXCLUDED.sync_source IN ('jellyfin', 'videasy') THEN
        ...
        percentage_watched = CASE
            -- Jellyfin and Videasy send absolute percentage; use the greater value
            WHEN EXCLUDED.sync_source IN ('jellyfin', 'videasy') THEN
                LEAST(GREATEST(

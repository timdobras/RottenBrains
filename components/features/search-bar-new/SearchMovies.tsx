...
import { useState, ChangeEvent, useEffect, useCallback, forwardRef, useMemo } from 'react';
...
const SearchMovies = forwardRef<HTMLInputElement, any>(({ media, setMedia, link, user }, ref) => {
      ...
      debounce(async (q: string) => {
        ...
        try {
          if (q.startsWith('@')) {
            ...
            const typedUserResults: any[] | undefined = userResults?.map((u: any) => ({
              ...u,
              type: 'user',
            }));
            ...
            const typedMediaResults: any[] = mediaResults.results.map((m: any) => ({
              ...m,
              type: 'media',
            }));
  ...
  const renderOptions = () => {
    ...
    if (!results?.length && query) {
      return <div className="px-2 py-1 text-sm/6 text-white">No results found.</div>;
    }

...
const InfiniteScrollByGenre: React.FC<InfiniteScrollByGenreProps> = ({ genre_id, media_type }) => {
  const { user } = useUser();
  ...
  const loadMore = useCallback(async () => {
    if (inView && hasMore && !loading) {
      setLoading(true);
      try {
        const result = await getFromGenres(media_type, page, genre_id.toString());
        ...
        if (resultsWithMediaType.length === 0 || page === 10) {
          ...
          setMediaItems((prevMediaItems) => [...prevMediaItems, ...resultsWithMediaType]);
  ...
  return (
    <div className="flex w-full flex-col justify-center gap-4 p-4 md:p-0" ref={targetRef}>

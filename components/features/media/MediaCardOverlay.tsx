  ...
  return (
    <>
      ...
      <div className="absolute bottom-2 right-2 flex flex-row-reverse gap-2">
        ...
        {number_of_episodes && <MediaInfoText text={`${number_of_episodes} Episodes`} />}
      ...
      <div className="absolute left-0 top-0 m-2">
        {isNew && (
          <div className="rounded-[4px] bg-secondary px-2 py-1 text-xs text-white">NEW</div>
        )}
        {isSoon && (
          <div className="rounded-[4px] bg-secondary px-2 py-1 text-xs text-white">SOON</div>
        )}
        {isNewEpisodes && (

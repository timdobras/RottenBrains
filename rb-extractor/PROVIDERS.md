...
```
Cracking the few real upstreams (**vidrock, videasy, 1shows**) covers most front-ends.
...
- **Direct cracking: tractable ones DONE** (vidlink, spencerdevs, vidrock). Remaining (videasy, 111movies) are deep-obfuscation/anti-tamper walls — keep on the (optimized) browser path. videasy CF is solved (curl_cffi) if we later defeat its anti-tamper decrypt.

...
export const searchTv = async (query: string, page: number = 1) => {
  return fetchFromApi(`search/tv?query=${query}&include_adult=false&language=en-US&page=${page}`);
};
...
export const getMediaDetails = async (
  ...
  if (media_id <= 0) {
    throw new Error(`Invalid media_id: ${media_id}. It must be a positive number.`);
  }
  ...
  if (
    ...
    throw new Error('Both season_number and episode_number must be provided together.');
  }
...
export const getRecommendations = async (media_type: string, media_id: number) => {
  return fetchFromApi(`${media_type}/${media_id}/recommendations?language=en-US&page=1`, 'images');
};

export const getSimilar = async (media_type: string, media_id: number) => {
  return fetchFromApi(`${media_type}/${media_id}/similar?language=en-US&page=1`);
};

export const getReviews = async (media_type: string, media_id: number) => {
  return fetchFromApi(`${media_type}/${media_id}/reviews?language=en-US&page=1`);
};
...
export const getFromGenres = async (media_type: string, page: number, genre: string) => {
  return fetchFromApi(
...
export async function getLastEpisodeFromTMDB(tvId: number): Promise<any | null> {
  try {
    ...
    const { air_date, season_number, episode_number } = data.last_episode_to_air;
